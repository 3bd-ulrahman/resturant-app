    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <form action="{{ route('dashboard.reservations.store') }}" method="POST">
        ...
        <div class="mb-6">
          ...
          <input class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full dark:focus:ring-0"
            name="date"
            value="{{ old('date', now()->format('Y-m-d\TH:i:s')) }}"
            type="datetime-local"
            autofocus
            min="{{ now() }}"
            max="{{ now()->createFromTimeString('17:00:00')->addWeek() }}"
            autocomplete="off"
          />
          @error('date')
        ...
        <div class="mb-6">
          ...
          <select name="table_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-900 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            autocomplete="off">
